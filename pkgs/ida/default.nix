{
    lib,
    stdenvNoCC,
    fetchurl,
    unzip,
    python3,
    perl,
    username,
    useremail,
}:

stdenvNoCC.mkDerivation rec {
    pname = "IDA Pro";
    version = "9.1";

    src = fetchurl {
        url = "https://vaclive.party/software/ida-pro/releases/download/9.1.250226/ida-pro_91_armmac.app.zip";
        sha256 = "sha256-jZF6lelsic4ASEguO6W9WW1P5/t3AOe/4F7GIfr1NaA=";
    };
    nativeBuildInputs = [ unzip python3 perl ];
    sourceRoot = ".";
    installPhase = ''
        runHook preInstall
        mkdir -p $out/Applications
        echo "Installing IDA Pro..."
        ./ida-pro_91_armmac.app/Contents/MacOS/installbuilder.sh --mode unattended --unattendedmodeui none --prefix $out/Applications/

        # patch to cracked
        function hex() {
            echo -n "$1" | perl -0777pe 's|([0-9A-Za-z]{2}+(?![^\(]*\)))|\\x\$\{1\}|gs'
        }

        function replace() {
            declare -r dom=$( hex $2 )
            declare -r sub=$( hex $3 )
            perl -0777pi -e 'BEGIN{$/=\1e8} s|'$dom'|'$sub'|gs' "$1"
            return
        }
        function prep() {
            echo "Resigning $1 ..."
            /usr/bin/xattr -cr "$1"
            /usr/bin/xattr -r -d com.apple.quarantine "$1"
            /usr/bin/codesign --force --deep --sign - "$1"
        }
        # backup
        echo "Backing up original files..."
        cp "$out/Applications/IDA Professional 9.1.app/Contents/MacOS/libida.dylib" \
            "$out/Applications/IDA Professional 9.1.app/Contents/MacOS/libida.dylib.bak"
        cp "$out/Applications/IDA Professional 9.1.app/Contents/MacOS/libida32.dylib" \
            "$out/Applications/IDA Professional 9.1.app/Contents/MacOS/libida32.dylib.bak"
        cp "$out/Applications/IDA Professional 9.1.app/Contents/Resources/hvui.app/Contents/MacOS/hv" \
            "$out/Applications/IDA Professional 9.1.app/Contents/Resources/hvui.app/Contents/MacOS/hv.bak"
        cp "$out/Applications/IDA Professional 9.1.app/Contents/Resources/hvui.app/Contents/MacOS/hvui" \
            "$out/Applications/IDA Professional 9.1.app/Contents/Resources/hvui.app/Contents/MacOS/hvui.bak"
        echo "Patching files..."
        echo "Patching libida.dylib..."
        replace "$out/Applications/IDA Professional 9.1.app/Contents/MacOS/libida.dylib" \
            '' \
            ''
        replace "$out/Applications/IDA Professional 9.1.app/Contents/MacOS/libida.dylib" \
            '602E40F900013FD6A0040034' \
            '602E40F900013FD625000014'
        replace "$out/Applications/IDA Professional 9.1.app/Contents/MacOS/libida.dylib" \
            'F800000085C00F8492' \
            'F800000085C090E992'
        echo "Patching libida32.dylib..."
        replace "$out/Applications/IDA Professional 9.1.app/Contents/MacOS/libida32.dylib" \
            '' \
            ''
        replace "$out/Applications/IDA Professional 9.1.app/Contents/MacOS/libida32.dylib" \
            '602E40F900013FD6A0040034' \
            '602E40F900013FD625000014'
        replace "$out/Applications/IDA Professional 9.1.app/Contents/MacOS/libida32.dylib" \
            'F800000085C00F8492' \
            'F800000085C090E992'
        echo "Patching hv..."
        replace "$out/Applications/IDA Professional 9.1.app/Contents/Resources/hvui.app/Contents/MacOS/hvui" \
            '' \
            ''
        replace "$out/Applications/IDA Professional 9.1.app/Contents/Resources/hvui.app/Contents/MacOS/hvui" \
            '602E40F900013FD6A0040034' \
            '602E40F900013FD625000014'
        replace "$out/Applications/IDA Professional 9.1.app/Contents/Resources/hvui.app/Contents/MacOS/hvui" \
            'F800000085C00F8492' \
            'F800000085C090E992'
        echo "Patch Completed."
        echo "Resigning patched files..."

        # prep "$out/Applications/IDA Professional 9.1.app"
        # prep "$out/Applications/IDA Professional 9.1.app/Contents/MacOS/libida.dylib"
        # prep "$out/Applications/IDA Professional 9.1.app/Contents/MacOS/libida32.dylib"
        # prep "$out/Applications/IDA Professional 9.1.app/Contents/Resources/hvui.app/Contents/MacOS/hv"
        # prep "$out/Applications/IDA Professional 9.1.app/Contents/Resources/hvui.app/Contents/MacOS/hvui"
        # prep "$out/Applications/IDA Professional 9.1.app/Contents/Resources/hvui.app"

        echo "Done."
        echo "Generating license file..."
        ${python3}/bin/python3 ${./patch.py} ${username} ${useremail}
        cp idapro.hexlic $out/Applications/IDA\ Professional\ 9.1.app/Contents/MacOS/idapro.hexlic
        runHook postInstall
    '';
    meta = with lib; {
        description = "IDA Pro leaked prebuilt binary for macOS (Darwin)";
        homepage = "https://hex-rays.com/";
        license = licenses.unfree;
        platforms = platforms.darwin;
    };
}
